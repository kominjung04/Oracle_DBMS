...
-- name,id,grade 필드 내용 출력
select name,id,grade from student;

-- deptno1 필드 중복 값 제거
select Distinct deptno1  from student;

-- deptno1 필드 오름차순 grade 필드 내림차순정렬
select *  from student order by deptno1,grade desc;

--name,id,grade필드출력 별칭 이름 아이디 학년설정
select name 이름,id 아이디,grade 학년 from student;

--select column list (col1,col2,col3)
--from table;

select *from emp;

--------where :: 조건 ----------
--sal이 3000이상인 데이터를 ename,sal출력
select ename,sal from emp where sal >=3000;

--job이 clerk인 데이터의 ename,job,sal출력
select ename,job,sal from emp where job = 'CLERK';

--MGR이 7698인 데이터의 ename,mgr,sal출력
select ename,mgr,sal from emp where mgr = 7698;

--EMPNO가 7800보다 작은 데이터를 출력
select * from emp where EMPNO < 7800;

--입사일이 81/06/06 이후인 데이터를 출력
select * from emp where hiredate>'81/06/06';
SELECT JOB,SAL FROM EMP;

--and와 or
--job이 SALESMAN이고 급여가 3000 이상인 데이터의 ENAME,JOB,SAL을 출력 
SELECT ENAME,JOB,SAL FROM EMP WHERE JOB='SALESMAN' AND SAL >=3000;

--MGR이 7698인 데이터 중 부서가 30번인 데이터 ENAME,MGR,DEPTNO를 출력
SELECT ENAME,MGR,DEPTNO FROM EMP WHERE MGR = 7698 AND DEPTNO = 30;

--HIREATE가 '1981/07/01'보다 크거나 급여가 3000보다 작은 데이터를 출력
SELECT * FROM EMP WHERE HIREDATE > '1987/07/01' OR SAL <3000;

--DEPTNO가 10 또는 30이고 급여가 3000이상인 데이터를 출력
SELECT * FROM EMP WHERE (DEPTNO =10 OR DEPTNO = 30) AND SAL >=3000;
...
--NOT 논리부정연산자--
select * from emp where sal<>3000; --sal 이 3000이 아닌 데이터
select * from emp where sal!=3000; --sal 이 3000이 아닌 데이터
select * from emp where sal^=3000; --sal 이 3000이 아닌 데이터
...
--부서번호가 1001,1003,1007인 사원의 정보를 출력하라
select * from emp2 where deptno in (1001,1003,1007);
--취미가 수영,낚시,등상인 사원의 정보를 출력하라
select * from emp2 where hobby in ('Fishing','Swim','Climb');
...
SELECT * FROM EMP WHERE ENAME BETWEEN 'C' AND 'K'; --이상 미만
SELECT * FROM EMP WHERE SAL NOT BETWEEN 2000 AND 3000;
SELECT * FROM EMP WHERE ENAME NOT BETWEEN 'C' AND 'K'; --초과 이하
SELECT * FROM EMP WHERE ENAME LIKE '_L%'; --L앞에 한개의 문자만 있고 L뒤에는 상관X
SELECT * FROM EMP WHERE ENAME LIKE '%AM%'; --AM이 포함된 문자
SELECT * FROM EMP WHERE ENAME NOT LIKE '%AM%';--AM이 포함되지 않은 문자

SELECT * FROM emp WHERE ENAME LIKE 'A\_A%' ESCAPE '\'; -- A_A로 시작하는 것만 매칭 ACA,ABAA X
--is null--
select * from emp where comm = NULL; --실행결과 없음
select * from emp where comm is NULL; -- 실행결과 있음
select * from emp where comm != NULL;
select * from emp where comm is not null and mgr is not null;
select * from emp where sal>NULL;

---UNION :: 열의 개수 와 대응하는 출력 자료형 같아야 함---
--열의 개수 같을 때--
...
minus
select empno,ename,job,mgr,hiredate,sal,comm,deptno from emp where sal>=2000 and  sal<3000;
select ename,empno,sal,deptno from emp where ename like '%E%' and deptno = 30 and sal not between 1000 and 2000;
select * from emp where comm is null and job in ('CLERK','MANAGER') and ename Not Like '_L%';
